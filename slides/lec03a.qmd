...
subtitle: Root finding methods for one dimensional optimisation
author:
...
:::: {.column}
Given $e$ (eccentricity) and $M$ (mean anomaly at a given time, in radians), find $E^*$ (eccentric anomaly).
...
::: {#rem-iteration}
We briefly comment that iteration is *different* to the idea of time stepping we met when solving differential equations.
...
::: {.columns}
:::: {.column}
The first method that we introduce is the bisection method.
If a function is smooth (continuous) on an interval and changes sign between the two end points (either positive to negative or negative to positive), then it must be zero some point in between.
::::
:::: {.column}
...
```{python}
# | echo: True
# | code-line-numbers: "1-5|6-7|11-22|13|14-15|16-20"
...
axes[0].plot(df["a"], ".-", label="$a$")
axes[0].plot(df["b"], ".-", label="$b$")

axes[0].set_xlabel("iterations")
axes[0].legend()
axes[0].grid(True)

axes[1].semilogy(abs(df["f(a)"]), ".-", label="$|f(a)|$")
axes[1].semilogy(abs(df["f(b)"]), ".-", label="$|f(b)|$")
axes[1].semilogy(df["b"] - df["a"], ".-", label="$b-a$")
...
axes[1].semilogy(df["f(a)"], ".-", label="$f(a)$")
axes[1].semilogy(df["f(b)"], ".-", label="$f(b)$")
axes[1].semilogy(df["b"] - df["a"], ".-", label="$b-a$")
...
```{python}
# | echo: True
# | code-line-numbers: "1-4|7-9|12|13-16"
...
axes[0].plot(df["x"], ".-", label="$x$")

axes[0].set_xlabel("iterations")
axes[0].set_ylabel("$x^{(i)}$")
axes[0].grid(True)

axes[1].semilogy(abs(df["f(x)"]), ".-", label="$|f(x)|$")
axes[1].set_xlabel("iterations")
axes[1].set_ylabel("$|f(x^{(i)})|$")
axes[1].grid(True)
...
```{python}
# | fig-cap: Example with slower convergence from $f'(x^*) = 0$
def f4(x):
    return x**2 - 2 * x + 1
...
::: {.notes}
The derivative is $f'(x) = \frac{1}{3} x^{-2/3}$ (note not defined at $0$) and the Newton iteration is
$$
...
```{python}
# | fig-cap: Example of non-convergence
def df5(x):
