...
- There are now lab sessions again -- check your timetable on Fri 27 March
...
2. Planning and scheduling complicated **robotic** tasks requires solving dynamic models for how the robotic will interact with its environment;
...
4. **Weather and climate** predictions are mode through solving very large dynamic models;
...
t_, s_ = np.zeros(2 * N - 1), np.zeros(2 * N - 1)
dt = T / (N - 1)
s_[0] = S(0.0)
for i in range(1, N):
    t_[2 * i - 1], t_[2 * i] = i * dt, i * dt
    s_[2 * i - 1], s_[2 * i] = S((i - 1) * dt), S(i * dt)
s_[-1] = S(T)
...
def plot(dt=1.0):
    ...
    plt.plot([0, dt], [bump_func(0), bump_func(dt)], label=f"{slope=:.1f}")
...
The *net acceleration downwards* (i.e. rate of change of speed, $S'(t)$) is given by the sum of the two forces $g$ and $-k S(t)$:
$$
S'(t) = g - k S(t).
$$
...
def freefall(n):
    """
    Compute the trajectory of an object falling freely.
...
The algorithm has one free parameter: the number of time steps, $n$. Once the number of time steps is fixed, we can calculate the time step $h = (T - t_0) / (n-1)$.
...
::: {#exr-euler}
Complete the next two time steps from to find an approximation of the solution at $T = 3$.
:::
