...
```{python}
# | fig-cap: Plots of solution using Euler's method
...
df["Ratio"] = df["Ratio"].fillna("---")

print(table_styler(df))
```
...
```{python}
# | fig-cap: Plot of errors for Euler's method not using logarthmic scales.
fig, axs = plt.subplots(1, 2)
axs[0].semilogx(
    error_table["h"], [8.0 for _ in error_table["h"]], label="exact"
)
...
We start with $t^{{(0)}} = {sp.latex(t0)}$ and $y^{{(0)}} = {sp.latex(y0)}$.
...
- Step {step + 1}:
    \\begin{{align*}}
      ...
      \\\\
      & = {sp.latex(y)} + \\frac{{{sp.latex(h)}}}{{2}} \\times
      {f_str(sp.latex(t), sp.latex(y))} \\\\
      & = {sp.latex(y)} + {sp.latex(h / 2)} \\times {sp.latex(f(t, y))}
      = {sp.latex(k)} \\\\
      m & = t^{{({step})}} + \\frac{{h}}{{2}} \\\\
      & = {sp.latex(t)} + \\frac{{{sp.latex(h)}}}{{2}} = {sp.latex(m)} \\\\
      y^{{({step + 1})}} & = y^{{({step})}} + h
       f(m, k) \\\\
      & = {sp.latex(y)} + {sp.latex(h)} \\times
      {f_str(sp.latex(m), sp.latex(k))} \\\\
      & = {sp.latex(y)} + {sp.latex(h)} \\times {sp.latex(f(m, k))}
      = {sp.latex(ynew)} \\\\
      t^{{({step + 1})}} & = t^{{({step})}} + h \\\\
      & = {sp.latex(t)} + {sp.latex(h)} = {sp.latex(tnew)}.
    \\end{{align*}}
    """
    s += s_step
...
```{python}
# | fig-cap: Plot of solutions using the midpoint method
fig, axs = plt.subplots(1, 2)
...
df["Ratio"] = df_mp["Ratio"].fillna("---")

print(table_styler(df_mp))
```
...
```{python}
# | fig-cap: Errors for the midpoint method and Euler's method
fig, axs = plt.subplots(1, 2)
...
print(
    table_styler(
        df_euler[["n", "h", "Run time", "Rhs evals", "Run time ratio"]]
    )
)
```
...
print(
    table_styler(
        df_midpoint[["n", "h", "Run time", "Rhs evals", "Run time ratio"]]
    )
)
...
```{python}
# | fig-cap: The computational cost of each method
fig, axs = plt.subplots(1, 2)

axs[0].loglog(df_euler["h"], df_euler["Run time"], ".-", label="Euler")
axs[1].loglog(df_euler["h"], df_euler["Rhs evals"], ".-", label="Euler")
axs[0].loglog(df_midpoint["h"], df_midpoint["Run time"], ".-", label="Midpoint")
axs[1].loglog(
    df_midpoint["h"], df_midpoint["Rhs evals"], ".-", label="Midpoint"
)
...
```{python}
# | fig-cap: The computational cost of each method against error
fig, axs = plt.subplots(1, 2)

axs[0].loglog(df_euler["Run time"], df_euler["Abs. error"], ".-", label="Euler")
axs[1].loglog(
    df_euler["Rhs evals"], df_euler["Abs. error"], ".-", label="Euler"
)
axs[0].loglog(
    df_midpoint["Run time"], df_midpoint["Abs. error"], ".-", label="Midpoint"
)
axs[1].loglog(
    df_midpoint["Rhs evals"], df_midpoint["Abs. error"], ".-", label="Midpoint"
)
