...
from utils import table_styler
...
::: {#rem-1d-opt}
In one dimension, a local minimum occurs at a point where $F'(x) = 0$, but not every root of $F'$ is a minimum. The focus of this lecture is finding possible minima.
:::

::: {#exm-root-finder}
1. The linear equation $f(x) = ax + b = 0$ has a single solution at $x^* = \frac{-b}{a}$.
2. The quadratic equation $f(x) = a x^2 + b x + c = 0$ is nonlinear, but simple enough to have a known formula for the roots:
   $$
...
def bisection(f, a, b, tol):
    ...
    -------
    data: list[dict]
        Iteration data for analysing results
...
data = bisection(f1, 1.0, 2.0, 1.0e-4)

table_styler(pd.DataFrame(data))
...
data = bisection(f2, 0.0, 2.0 * np.pi, 1.0e-4)
...
plt.plot(x_iter[0], f3(x_iter[0]), "C1o", label="$x^{(0)}$")
...
data = newton(f2, df2, np.pi, 1.0e-4)
...
\begin{align*}
x^{(1)} &= x^{(0)} - \frac{f(x^{(0)})}{f'(x^{(0)})}
= 0 - \frac{0^3 + 2 \times 0^2 + 0 + 1}{3 \times 0^2 + 4 \times 0 + 1} = 0 - \frac{1}{1} = -1.
\end{align*}
But we see that
$$
...
```{python}
def f5(x):
    # two part definition to avoid using complex numbers
    if x >= 0.0:
        return x ** (1 / 3)
    else:
        return -((-x) ** (1 / 3))
...
The derivative is $f'(x) = \frac{1}{3} x^{-2/3}$ (note this isn't defined at $x=0$) and the Newton iteration is
$$
...
data = newton(f5, df5, 1, 1e-4, maxiter=10)
...
```{python}
data = newton(f6, df6, 1, 1e-4, maxiter=10)
