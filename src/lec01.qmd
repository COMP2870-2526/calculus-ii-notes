...
```{python}
# | tbl-cap: "Computing the distance travelled using smaller steps."
# | label: tbl-distance-convergence
# | output: asis
...
print(table_styler(pd.DataFrame(data)))
```
...
\end{align*}
We compare this exact value to @tbl-distance-convergence, where for 100 000 intervals, we estimated the total distance to be $0.416669$.
...
The approach used in @exm-freefall can be applied to any differential equation in the standard format:
$$
y'(t) = f(t, y(t)) \quad\text{subject to the initial condition}\quad y(t_0) = y_0.
$$
...
$$
y(t_0 + h) \approx y(t_0) + \int_{t_0}^{t_0 + h} f(t_0, y_0) \, \mathrm{d}t.
$$
This gives us an integral, we can evaluate since the term inside the integral is constant:
\begin{align*}
y(t_0 + h) & \approx y(t_0) + (t_0 + h - t_0) f(t_0, y_0) \\
& = y(t_0) + h f(t_0, y_0).
\end{align*}
...
h = (T - t0) / n  # symbolic, gives Rational(1,2)

t, y = t0, y0


# Define f(t, y) symbolically
def f(t, y):
    return -(y**2) + 3 * t**2


def f_str(t, y):
    return f"-\\left({y}\\right)^2 + 3 \\times \\left({t}\\right)^2"
...
for step in range(n):
    ynew = sp.simplify(y + h * f(t, y))
    tnew = sp.simplify(t + h)

    s_step = f"""
- Step {step + 1}:
    \\begin{{align*}}
      y^{{({step + 1})}} &= y^{{({step})}} + h f(t^{{({step})}}, y^{{({step})}})
      \\\\
      &= {sp.latex(y)} + {sp.latex(h)} \\times
      \\left({f_str(sp.latex(t), sp.latex(y))}\\right) \\\\
      &= {sp.latex(y)} + {sp.latex(h)} \\times {sp.latex(f(t, y))} \\\\
      &= {sp.latex(ynew)} \\\\
      t^{{({step + 1})}} &= t^{{({step})}} + h \\\\
